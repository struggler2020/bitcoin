...
//我们通过分析发现，这个函数简单说就是，读取键值，根据key 的类型，把数据添加到钱包相应的数据中去。
{
    try {
        // Unserialize
...
DBErrors CWalletDB::LoadWallet(CWallet* pwallet)
{
    ...
    try {
        ...
        loop
        {
            ...
            //其中最主要的就是下面if里面的ReadKeyValue，显然是具体的分析函数。我们继续往里面看
...
void ThreadFlushWalletDB(void* parg)
{
    ...
    //很显然是实时刷新钱包数据的
    //这里我们不做过多的分析，而是放到后面的交易细节章节详细解释
    //应用程序初始化时，最后创建此线程，而且只有一次。默认是刷钱包
    //但可以通过flushwallet来是否刷新数据库。大概500ms刷新一次
    //当钱包数据有更新，且距离上次更新时间超过2，且钱包数据库文件
    //引用次数为0，则关闭数据库文件从而刷新数据。
    //更加详细的内容我们将在交易细节章说明。
    ...
    while (!fShutdown)
    {
		...
		//nWalletDBUpdated，当这个值置位的时候，则钱包数据库进行刷新。
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys)
{
    ...
    bool allOK = dbenv.Salvage(newFilename, true, salvagedData);//做数据的查询与存储，以后有时间将继续说明
    ...
    BOOST_FOREACH(CDBEnv::KeyValPair& row, salvagedData)
    {
        if (fOnlyKeys)
        {
            ...
            bool fReadOK = ReadKeyValue(&dummyWallet, ssKey, ssValue,
                                        ...
                                        strType, strErr);
			//ReadKeyValue,查找交易信息，以后有时间将继续说明
            if (!IsKeyType(strType))
                continue;
            if (!fReadOK)
            {
