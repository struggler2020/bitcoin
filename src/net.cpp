...
//下面对连续发送同一个getblocks的消息进行了过滤.pindexLastGetBlockBegin表示开始同步的位置，
//hashLastGetBlocksEnd表示结束的区块的哈希值。
//最后调用PushMessage发送"getblocks"同步当前区块链。
...
//注意除了addrConnect外，剩下的都是空
{
    if (pszDest == NULL) {
		//预处理
		//如果参数 pszDest 为空指针，则处理如下：
		//如果要连接的地址是本地地址，则直接返回空指针。
		
        if (IsLocal(addrConnect))
            return NULL;
	
        // Look for an existing connection
        //调用 FindNode 方法，查看指定的节点是否存在。
        //如果存在，即已经连接，则返回空指针。
    ...
    //开始正式的连接
    SOCKET hSocket;
	//在运算中，首先对第一个表达式pszDest进行检验，如果为真，则返回表达式2的值ConnectSocketByName；如果为假，则返回表达式3的值。
	//因此我们这里运行ConnectSocket(addrConnect, hSocket)函数，也只需要查看这个即可，这个函数也是关键函数，注意就一个有效值。
...
//第六节ThreadSocketHandler
//通过套接字收发数据的
//接收数据并处理(当某个节点的套接字可读时，将从套接字读取数据并把数据添加到节点的接收缓冲区)
//(最后，把接收缓冲区的数据拼接到待处理消息缓冲区，然后通知消息处理线程有新的消息需要处理)
//发送数据     当某个节点的套接字可写时将数据通过套接字发送出去
void ThreadSocketHandler2(void* parg)
{
		...
		//显然这是一个循环
    {
        ...
        //断开节点
        {
			...
			//断开没有使用的节点，首先遍历节点数组vNodesCopy，
			//如果节点标识断开连接(fDisconnect)，或者没有任何引用、发送接收消息，
			//则移除节点，关闭socket，添加到断开连接节点数组 (vNodesDisconnected)
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
			//遍历监听socket数组，当接收到数据，且socket有效时，
			//处理远程对等节点的连接逻辑
        {
#ifdef USE_IPV6
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
                ...
                if (lockRecv)
                {
                    ...
                    if (nPos > ReceiveBufferSize()) {
							...
							//正常接收并在接收信息时间上更新时间戳
                        {
							...
							//把pchBuf里面长度为nBytes的内容复制到&vRecv[nPos]
			...
			//距离上次发送消息超过了90分钟(nLastSend)，且距离上次把全部消息发送出去也超过了90分钟(nLastSendEmpty)。
			//距离上次接收消息超过了90分钟(nLastRecv)
			
        }
		//以上就是处理所有的发送和接收信息的源码分析。
...
void ThreadDNSAddressSeed2(void* parg)
{
    ...
    if (!fTestNet)
    {
		...
		//下面开始搜索strDNSSeed里面的dns地址
        for (unsigned int seed_idx = 0; seed_idx < ARRAYLEN(strDNSSeed); seed_idx++) {
            if (HaveNameProxy()) {
				//如果指定了代理，则调用 AddOneShot 方法，保存当前 DNS 种子节点到 vOneShots 集合中
				//因为我们默认不用代理，所以不用考虑
                AddOneShot(strDNSSeed[seed_idx][1]);
            } else {
                vector<CNetAddr> vaddr;//定义了一个ip地址
                vector<CAddress> vAdd;//定义了一个节点信息
                if (LookupHost(strDNSSeed[seed_idx][1], vaddr))
					//调用 LookupHost 方法，根据 DNS 种子节点获取其保存的对等节点列表。并保存在 vaddr 集合中。
					//我们从这个函数继续往下看
                {
                    BOOST_FOREACH(CNetAddr& ip, vaddr)
					//如果 LookupHost 方法返回结果为真，即根据当前 DNS 种子节点查找到了至少一个对等节点，则进行遍历处理。
					//遍历  集合，根据当前的 IP 地址，生成一个 CAddress 地址对象，并保存在 vAdd 集合中，
					//同时把代表找到节点的变量 found 加1。调用地址管理器的 Add 方法，保存多个地址。
                    {
                        int nOneDay = 24*3600;
                        CAddress addr = CAddress(CService(ip, GetDefaultPort()));
						//GetDefaultPort获取缺省端口，addr从其中获得地址
...
void ThreadOpenConnections2(void* parg)
{
    ...
    //这里指的是只进行-connect连接是如何进行的，我们跳过去。
    ...
    loop
    {
        ...
        //增加seednode节点，如果irc没有工作。
        //因为我们这里不涉及到与irc有关的，，所以这里不看。
        //这里主要是处理addr的存储问题，
        ...
        //这个地方我们理解为对连接地址的预处理即可，主要是用来控制连接的数量。
        int nOutbound = 0;
        ...
        loop
        {
            ...
            //我理解为对外连接的数量，从10到90，不知道是不是正确。
            CAddress addr = addrman.Select(10 + min(nOutbound,8)*10);
			//****addr是从addrman里面选择出来的
            ...
            //如果在尝试从addrman获取100个地址后找不到合适的目标，请停止此循环，
            //并在尝试新的addrman地址之前再次运行外部循环
            //然后后面就是几种需要停止的情况
            nTries++;
            if (nTries > 100)
                break;
            ...
            addrConnect = addr;
			//*****下面的OpenNetworkConnection处理的是这个函数
            break;
        }
		//最后的预处理结束
...
//所以本节简单说一下，就是通过无数的预处理，然后最终确定连接有效，并进行连接。
...
void ThreadOpenAddedConnections2(void* parg)
{
	...
	//注意这个名字
    BOOST_FOREACH(string& strAddNode, mapMultiArgs["-addnode"])//根据addnode进行处理
    {
        vector<CService> vservNode(0);
        if(Lookup(strAddNode.c_str(), vservNode, GetDefaultPort(), fNameLookup, 0))
		//如果找到，则进行下面的处理：生成要连接的地址对象。
		//请看一下这个函数
        {
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemaphoreGrant *grantOutbound, const char *strDest, bool fOneShot)
{
    ...
    if (fShutdown)
        return false;
    if (!strDest)
		//如果参数 strDest 为空（当前节点信息的地址），
		//进一步，如果要连接的节点是本地的，或是已连接的，或是禁止的，则返回。
        if (IsLocal(addrConnect) ||
    ...
    if (strDest && FindNode(strDest))
        return false;
	//如果参数 strDest 不为空，并且找到节点，则返回，因为为空，所以进一步。

    vnThreadsRunning[THREAD_OPENCONNECTIONS]--;
    CNode* pnode = ConnectNode(addrConnect, strDest);
	//注意这里的strDest仍是空。
	//调用 ConnectNode 方法，连接到指定地址，并返回对等节点 CNode 对象。如果连接失败，则返回。
    vnThreadsRunning[THREAD_OPENCONNECTIONS]++;
    if (fShutdown)
        return false;
    if (!pnode)
...
void ThreadMessageHandler2(void* parg)
{
    ...
    while (!fShutdown)
    {
        vector<CNode*> vNodesCopy;
        {
            LOCK(cs_vNodes);
            vNodesCopy = vNodes;
			//生成一个对等节点容器对象 vNodesCopy，类型为 CNode*。然后，设置其值为 vNodes 。
			//后者，代表当前节点保存的所有对等节点信息。
            BOOST_FOREACH(CNode* pnode, vNodesCopy)
                pnode->AddRef();
			//遍历所有的 vNodesCopy 节点，调用当前节点的 AddRef 方法，把对等节点的相关属性加1。
        }
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy)
        {
            ...
            // Send messages
            {
                TRY_LOCK(pnode->cs_vSend, lockSend);
                if (lockSend)
					//第八节从ThreadMessageHandler到SendMessages
                    SendMessages(pnode, pnode == pnodeTrickle);
				//该函数处理本对等节点发送给当前远程对等节点的消息
				//核心函数
				//我们重点对这两个函数进行解析
            }
            if (fShutdown)
...
void StartNode(void* parg)
{
    ...
    if (!GetBoolArg("-dnsseed", true))
		...
		//通过查询DNS节点来找到足够多的比特币节点。找到之后才可以连接比特币网络进行同步。
		//五大线程之二
		//（完毕）
	...
	//第四节ThreadOpenAddedConnections与ThreadOpenConnections
    // Initiate outbound connections from -addnode
    if (!NewThread(ThreadOpenAddedConnections, NULL))
        printf("Error: NewThread(ThreadOpenAddedConnections) failed\n");
	//处理rpc客户端添加的节点信息，然后连接到节点（完毕）
	//五大线程之三

    // Initiate outbound connections
    if (!NewThread(ThreadOpenConnections, NULL))
        printf("Error: NewThread(ThreadOpenConnections) failed\n");
	//根据addrman中的ip连接到对应节点，创建节点CNode（连接别人fInbound为false，被连接为true）。
	//默认最多连接到8个节点。默认连接总数不大于125个，包括被连接的节点数
	//启动节点连接线程，连接邻近节点（完毕）
	//五大线程之四
	...
	//我们在这里也看到，通过-gen，可以设置为直接启动，启动同时需要钱包文件
}
